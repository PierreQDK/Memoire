...
```{r}
# Calcul de la différence avant/après pour chaque groupe
mean_did_mai_diff <- mean_did_mai %>%
  spread(Période, Prix_moyen) %>%
  mutate(Différence = Après - Avant)

# Création du graphique avec la différence avant-après affichée
ggplot(mean_did_mai, aes(x = Période, y = Prix_moyen, group = Groupe, color = Groupe)) +
  geom_line(linewidth = 1.5) +
  geom_point(size = 3) +
  labs(title = "Effet estimé de la novation (DiD) - Contrats MAY",
       x = "Période", y = "Prix moyen") +
  theme_minimal() +
  # Ajout de la différence avant/après sur le graphique
  geom_text(data = mean_did_mai_diff, 
            aes(x = 1.5, y = max(mean_did_mai$Prix_moyen), 
                label = paste("Différence:", round(Différence, 2))),
...
# Filtrage des contrats MAY et JULY
contracts_may_july <- a1[grepl("MAY|JULY", a1$Indiv), ]
...
chicago_prices <- contracts_may_july %>%
  ...
  rename(Chicago_Price = Close)  # Renommer la colonne pour faciliter la fusion
...
# Calcul de la moyenne des spreads avant et après la réforme pour chaque mois
spread_avg_may <- contracts_may_july %>%
  filter(grepl("MAY", Indiv)) %>%
  group_by(Ville, Période) %>%
  summarise(Spread_Moyenne = mean(Spread, na.rm = TRUE), .groups = "drop")

spread_avg_july <- contracts_may_july %>%
  filter(grepl("JULY", Indiv)) %>%
  group_by(Ville, Période) %>%
  summarise(Spread_Moyenne = mean(Spread, na.rm = TRUE), .groups = "drop")

# Calcul de la différence des spreads pour Mai
spread_diff_may <- spread_avg_may %>%
  spread(key = Période, value = Spread_Moyenne) %>%
  mutate(Différence = `Après` - `Avant`)

# Calcul de la différence des spreads pour Juillet
spread_diff_july <- spread_avg_july %>%
  spread(key = Période, value = Spread_Moyenne) %>%
  mutate(Différence = `Après` - `Avant`)

# Calcul des résultats généraux (pour toutes les villes)
...
# Affichage des résultats pour Mai et Juillet
cat("Résultats pour Mai :\n")
print(spread_diff_may)

cat("\nRésultats pour Juillet :\n")
print(spread_diff_july)
