...
# 📊 Fonction pour afficher des statistiques propres
describe_clean <- function(data, varname) {
  vec <- data[[varname]]
  cat(paste0("\n--- Statistiques descriptives (nettoyées) pour ", varname, " ---\n"))
  print(summary(vec))
  cat("Écart-type :", round(sd(vec, na.rm = TRUE), 2), "\n")
}
...
summary_stats <- oc_data_clean %>%
  summarise(
    ...
    Médiane = median(Open_Commitment, na.rm = TRUE),
    Écart_type = sd(Open_Commitment, na.rm = TRUE),
